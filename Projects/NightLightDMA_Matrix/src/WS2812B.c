...
#define WS281x_NOP_TICKS               1

#define SRC_SIZE                       (WS281x_MATRIX_WIDTH * WS281x_MATRIX_HEIGHT * WS281x_COLOR_DEPTH + WS281x_NOP_TICKS)
//#define SRC_SIZE        1586
...
void WS281x_Update(uint32_t *src)
{
	...
	uint32_t LED_COUNT = WS281x_MATRIX_WIDTH * WS281x_MATRIX_HEIGHT;
...
static void TIM_Config(void)
{
  ...
  TIM_OC1Init(WS281x_TIMER, &TIM_OCInitStructure);
	...
	TIM_DMACmd(WS281x_TIMER, TIM_DMA_CC1, ENABLE);////TIM_DMA_Update
...
static void DMA_Config(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
	...
	DMA_ITConfig(WS281x_DMA_CHANNEL, DMA_IT_TC, ENABLE);

  /* Enable and set priority of WS281x_DMA_CHANNEL_IRQn Interrupt */
  NVIC_InitStructure.NVIC_IRQChannel = 11;//WS281x_DMA_CHANNEL_IRQn;
